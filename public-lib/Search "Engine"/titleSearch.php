...
if(isset($_POST['titleSearch'])) {
            if(isset($_GET['go'])) {
              if(preg_match("/^[a-zA-Z0-9]+/", $_POST["title"])){ 
                 ...
                 $sql = "SELECT paper.paperid, title, type, keyword, journal.name, Volume, Number, Pages, paper.year, link FROM paper inner join journal on journal.paperid = paper.paperid WHERE title LIKE '%".$name."%' ORDER BY title LIMIT 10"; 
              //-run  the query against the mysql query function 13	  
                ...
                $sql2 = "SELECT paper.paperid, title, type, keyword, conference.venue, conference.year, link FROM paper INNER JOIN Conference ON conference.paperid =  paper.paperid WHERE title LIKE '%".$name."%' ORDER BY title LIMIT 10";
