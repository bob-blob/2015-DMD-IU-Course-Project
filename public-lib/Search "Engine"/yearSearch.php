...
if(isset($_POST['yearSearch'])) {
            if(isset($_GET['go'])) {
              if(preg_match("/^[0-9]+/", $_POST["year"])){ 
                 ...
                 $sql = "SELECT paper.paperid, title, type, keyword, journal.name, Volume, Number, Pages, paper.year, link FROM paper inner join journal on journal.paperid = paper.paperid WHERE paper.year LIKE '".$name."%' ORDER BY title LIMIT 10"; 
              //-run  the query against the mysql query function 13	  
                  ...
                  while ($line = pg_fetch_array($result, null, PGSQL_ASSOC)) {
                    ...
                    echo "\t<div class='blockEdit'><a href='index.php?page=edit&pid=$id&kind=1'><div class='edit'>EDIT</div></a><a href='index.php?page=delete&id=$id'><div class='del'>DELETE</div></a></div></div></div>s\n";
                }
                
                        
                $sql2 = "SELECT paper.paperid, title, type, keyword, conference.venue, conference.year, link FROM paper INNER JOIN Conference ON conference.paperid =  paper.paperid WHERE paper.year LIKE '%".$name."%' ORDER BY title LIMIT 10";
