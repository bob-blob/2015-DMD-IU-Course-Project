...
if(isset($_POST['journalSearch'])) {
            if(isset($_GET['go'])) {
              if(preg_match("/^[a-zA-Z0-9]+/", $_POST["journal"])){ 
                  
                  $name=$_POST["journal"]; 
                            
                  //-query  the database table 
                  
                 $sql = "SELECT paper.paperid, title, type, keyword, journal.name, Volume, Number, Pages, paper.year, link FROM paper inner join journal on journal.paperid = paper.paperid WHERE journal.name LIKE '%".$name."%' ORDER BY journal.name LIMIT 10"; 
