...
if(isset($_POST['instituteSearch'])) {
            if(isset($_GET['go'])) {
              if(preg_match("/^[a-zA-Z0-9]+/", $_POST["institute"])){ 
                  
                  $name=$_POST["institute"]; 
                            
                  //-query  the database table 
                  
                 $sql = "SELECT paper.paperid, title, type, keyword, journal.name, Volume, Number, Pages, paper.year, link FROM author NATURAL JOIN published_paper NATURAL JOIN paper inner join journal on journal.paperid = paper.paperid WHERE workplace LIKE '%".$name."%' ORDER BY workplace"; 
              //-run  the query against the mysql query function 13	  
          
                  $result=pg_query($db, $sql);
                  
                  
                   echo "\t<div class='headYear'>Found with workplace: $name</div>\n";
                ...
                $sql2 = "SELECT paper.paperid, title, type, keyword, conference.venue, conference.year, link FROM author NATURAL JOIN published_paper NATURAL JOIN paper INNER JOIN Conference ON conference.paperid =  paper.paperid WHERE workplace LIKE '%".$name."%' ORDER BY workplace LIMIT 10";
